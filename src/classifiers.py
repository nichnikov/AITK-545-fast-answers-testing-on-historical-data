...
def search_result_rep(search_result: list):
    return [{**d["_source"],
             **{"id": d["_id"]},
...
class FastAnswerClassifier:
    ...
    def sbert_ranging(self, lem_query: str, score: float, candidates: []):
        ...
        if the_best_result[4] >= score:
            return {"sbert_best_id": the_best_result[0], "sbert_best_etalon": the_best_result[1], 
                    "sbert_best_answer": the_best_result[3], "sbert_score": the_best_result[4]}
    ...
    def t5_validate(self, query: str, answer: str, score: float):
        ...
        return {"T5Opinion": val_str, "T5Score": t5_score}
            ...
            if tokens[0]:
                ...
                if search_result["hits"]["hits"]:
                    ...
                    if sbert_the_best_result: 
                        validate_result = self.t5_validate(tokens_str, sbert_the_best_result["sbert_best_answer"], t5_score)
                        answer = await self.get_answer(sbert_the_best_result["sbert_best_id"], pubid)
                        return {**answer, **sbert_the_best_result, **validate_result}
                    else:
